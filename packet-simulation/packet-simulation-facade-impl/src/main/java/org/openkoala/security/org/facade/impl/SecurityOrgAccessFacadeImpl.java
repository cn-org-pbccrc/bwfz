...
public class SecurityOrgAccessFacadeImpl implements SecurityOrgAccessFacade {
    ...
    private List<EmployeeUserDTO> transformToEmployeeUserDTO(List<EmployeeUser> employeeUsers) {
        ...
        for (EmployeeUser employeeUser : employeeUsers) {
            ...
            System.out.println("嘻嘻哈"+employeeUser.getCreateOwner());
            if(employeeUser.getCreateOwner().equals("system")){
            	result.setDepartmentName("-");
            	System.out.println("1111111111111111111");
            }else{
            	if(employeeUser.getOrganization().equals("1")){
            		result.setDepartmentName(employeeUser.getCompany().getName());
            		System.out.println("啊哈哈哈哈哈哈哈哈哈哈哈哈哈哈");
                	System.out.println(employeeUser.getCompany());
            	}else{
            		result.setDepartmentName(employeeUser.getDepartment().getName());
                	System.out.println("22222222222222");
                	System.out.println(employeeUser.getDepartment());
            ...
            if(employeeUser.getEmployee() != null){
                ...
                //result.setDepartmentName(employeeUser.getDepartment().getName());
                //System.out.println("哈哈找到啦:"+employeeUser.getDepartment().getName());
...
//        }
        if (!StringUtils.isBlank(queryEmployeeUserCondition.getOrganization())) {
            jpql.append("AND .organization LIKE :organization");
            conditionVals.put("organization", MessageFormat.format("%{0}%", queryEmployeeUserCondition.getOrganization()));
        }
//        if (!StringUtils.isBlank(queryEmployeeUserCondition.getDepartmentName())) {
//          jpql.append("AND .department.name LIKE :departmentName");
//          conditionVals.put("departmentName", MessageFormat.format("%{0}%", queryEmployeeUserCondition.getDepartmentName()));
//        }
    }
